...
if [ -f "$OPTIONS_FILE" ]; then
  ...
  WOL_REPEATS=$(jq -r '(.wol_repeats // .options.wol_repeats) // 2' "$OPTIONS_FILE")
...
export LOG_LEVEL WOL_PORT WOL_REPEATS BROADCAST_IP LISTEN_PORT SECURE_ON KNOWN_HOSTS HOST_FILTERING MAC_LIST MAC_FILTERING DNS_TTL HTTP_API_EXPOSE API_PORT WEBHOOK_ID HA_API_URL
